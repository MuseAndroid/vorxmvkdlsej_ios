...
- (void)viewDidLoad {
    ...
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"loginParam"];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"CaptcahImg"];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"certFlag"];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"comCode"];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"empCode"];
        ...
        [FFNetworkManager url:[NSString stringWithFormat:@"%@%@", [NSString api_baseURL], @"/checkMobileLogin.ajax"] parameter:parameters success:^(NSDictionary *data) {
            if (![data isEqual:nil]) {
                ...
                if (![chkAutoLogin isEqualToString:@"0"]) {
                    ...
                    // 자동로그인 false
                    [self timeInterval:chkAutoLogin];
                }
            }
