...
- (instancetype) init
{
    ...
    if (self) {
        ...
        if ([[NSUserDefaults standardUserDefaults] objectForKey:@"comCode"]) {
            ...
            if ([[NSUserDefaults standardUserDefaults] objectForKey:@"secondaryComCD"]) {
            _comCodeTxtField.text = [[NSUserDefaults standardUserDefaults] objectForKey:@"secondaryComCD"];
                }
        }
        ...
        if ([[NSUserDefaults standardUserDefaults] objectForKey:@"empCode"]) {
            ...
            if ([[NSUserDefaults standardUserDefaults] objectForKey:@"secondaryConstID"]) {
            _empCodeTxtField.text = [[NSUserDefaults standardUserDefaults] objectForKey:@"secondaryConstID"];
            }
        }
...
{
    NSString *savedComCode = [[NSUserDefaults standardUserDefaults] objectForKey:@"comCode"];
    if ([savedComCode length] == 0) {
        [[NSUserDefaults standardUserDefaults] setObject:_comCodeTxtField.text forKey:@"comCode"];
        [[NSUserDefaults standardUserDefaults] setObject:_empCodeTxtField.text forKey:@"empCode"];
    }
...
{
    NSLog(@"넘어온 회사코드 = %@", comCode);
    if ([comCode length] != 0) {
        _comCodeTxtField.text = comCode;
    }
}

- (void) setEmpCode:(NSString *)empCode
{
    NSLog(@"넘어온 사원코드 = %@", empCode);
    if ([empCode length] != 0) {
        _empCodeTxtField.text = empCode;
    }
}
@end
