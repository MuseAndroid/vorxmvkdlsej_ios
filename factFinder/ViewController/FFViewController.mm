...
@property (strong, nonatomic) FFCaptcahViewController *captchaVC;
...
- (void)viewDidLoad {
    ...
    _captchaVC = [[FFCaptcahViewController alloc] init];
...
//    [FFUtility showToastCompleteHandler:@"크롤링 완료!!!" :self];
    ...
    apiUrl = [NSString stringWithFormat:@"%@%@", [NSString api_baseURL], @"/resultJsonSave_for_mobile.ajax"];
    ...
    if ([rOrgCD isEqualToString:@"credit4u"]) {
        ...
        if ([mb_id isEqualToString:@"gnx"]
            || [mb_id isEqualToString:@"GNX"]
            || [mb_id isEqualToString:@"mfs"]
            || [mb_id isEqualToString:@"MFS"]) {
            slideGubun = @"3";
        } else {
            slideGubun = @"2";
                  ...
                  :(NSString *)viewGubun
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSString *genderInt = @"";
        if ([genderIntTmp isEqualToString:@"1"] || [genderIntTmp isEqualToString:@"3"] || [genderIntTmp isEqualToString:@"5"] || [genderIntTmp isEqualToString:@"7"]) {
            genderInt = @"1";
        } else if ([genderIntTmp isEqualToString:@"2"] || [genderIntTmp isEqualToString:@"4"] || [genderIntTmp isEqualToString:@"6"] || [genderIntTmp isEqualToString:@"8"]) {
            genderInt = @"0";
        } else {
            genderInt = @"1";
        }
...
//            // 이 부분에서 캡챠코드 이미지를 이미지뷰에 넘기는 코드 작성
            ...
            NSLog(@"결과2 = %@", captcah);
            NSLog(@"============================================");
//            [self->_captchaVC refreshCaptchaImg:imagem :viewGubun];
            
            if ([viewGubun isEqualToString:@"crawling"]) {
                _captchaVC = [[FFCaptcahViewController alloc] init];
                [self presentViewController:_captchaVC animated:YES completion:nil];
            } else {
//                [_captchaVC set]
                [_captchaVC refreshCaptchaImg:imagem :@""];
            }
        });
    });
}
                  ...
                  :(NSString *)nationCD
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [NSUserDefaults.standardUserDefaults setBool:true forKey:@"certFlag"];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if ([json[@"errYn"] isEqualToString:@"Y"]) {
                [NSUserDefaults.standardUserDefaults setObject:@"Y" forKey:@"errYn"];
                NSString *consultant_id = @"";
                NSString *mb_id = @"";
                NSString *screenInfo = [[NSUserDefaults standardUserDefaults] objectForKey:@"loginScreen"];
                if ([screenInfo isEqualToString:@"기업고객"]) {
...
//        }
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if ([json[@"errYn"] isEqualToString:@"Y"]) {
                ...
                [self addText : ret];
                [_crawlingVC returnWebMainTimer];
